            ...
            unsafe
            {
                fixed(byte*ptr = &colors[0])
                {
                    ...
                    GL.TexImage2D(TextureTarget.Texture2D, 0,
                        PixelInternalFormat.Rgba, width, height, 0, PixelFormat.Rgba, PixelType.UnsignedByte, intPtr);
                    GraphicsExtensions.CheckGLError();
                }
            }
        }
        ...
        public byte[] GetData()
        {
            ...
            unsafe
            {
                fixed(byte*ptr = &data[0])
                {
                    ...
                    GL.GetTexImageInternal(TextureTarget.Texture2D, 0, PixelFormat.Rgba, PixelType.UnsignedByte, intPtr);
                }
            }
            GraphicsExtensions.CheckGLError();
            return data;
