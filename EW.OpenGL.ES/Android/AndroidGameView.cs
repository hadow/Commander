...
namespace EW.OpenGLES.Mobile
{
    ...
    public class AndroidGameView : SurfaceView, ISurfaceHolderCallback, View.IOnTouchListener
    {
        ...
        public class BackgroundContext
        {
            ...
            {
                this.view = view;
                foreach (var v in EW.OpenGLES.GLESVersion.GetSupportedGLESVersions())
                {
        ...
        protected void CreateGLContext()
        {
            ...
            var createdVersion = new EW.OpenGLES.GLESVersion();
            foreach (var v in EW.OpenGLES.GLESVersion.GetSupportedGLESVersions())
            {
        ...
        protected void CreateGLSurface()
        {
            if (!glSurfaceAvailable)
            {
                try
                {
                    ...
                    if (EW.OpenGLES.GL.GetError == null)
                        EW.OpenGLES.GL.LoadEntryPoints();
