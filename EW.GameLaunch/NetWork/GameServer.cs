...
namespace EW.NetWork
{
    ...
    public class GameServer
    {
        ...
        public GameServer(MiniYaml yaml)
        {
            ...
            if (WarGame.ExternalMods.TryGetValue(externalKey, out external) && external.Version == Version)
            {
                ModLabel = "{0} ({1})".F(external.Title, external.Version);
                IsCompatible = true;
            }
            else if (WarGame.Mods.TryGetValue(Mod, out mod))
            {
            ...
            else
            {
                ...
                var guessMod = WarGame.ExternalMods.Values
            ...
            var mapAvailable = WarGame.Settings.Game.AllowDownloading || WarGame.ModData.MapCache[Map].Status == MapStatus.Available;
