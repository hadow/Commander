    ...
    public sealed class VoxelLoader:IDisposable
    {
        ...
        static SheetBuilder CreateSheetBuilder()
        {
            ...
            Func<Sheet> allocate = () =>
            {
                ...
                allocated = true;
                return SheetBuilder.AllocateSheet(SheetT.Indexed, WarGame.Settings.Graphics.SheetSize);
            };

            return new SheetBuilder(SheetT.Indexed, allocate);
        }
        ...
        Vertex[] GenerateSlicePlane(int su, int sv, Func<int, int, VxlElement> first, Func<int, int, VxlElement> second, Func<int, int, Vector3> coord)
        {
        ...
        public void RefreshBuffer()
        {
            ...
            vertexBuffer = WarGame.Renderer.CreateVertexBuffer(totalVertexCount);
